...
public class StickyPiston extends Piston {
    ...
    protected void onRetract(World world) {
        ...
        if (blockToPull != null && !isUnpushable(targetPos, blockToPull)) {
            ...
            if (LogicaConstants.isLogicaComponent(blockToPull)) {
                ILogicaComponent comp = LogicaNetworkManager.getInstance()
                        .getComponentAt(targetPos);
                if (comp instanceof NetComp netComp) {
                    netComp.setPosition(destPos);
                }
            }
